...
class BigQueryMgr {
    ...
    insertDBNoBuffer(table, values) {
        return __awaiter(this, void 0, void 0, function* () {
            const strInsParticipant = `STRUCT('${values.participants[0].nom}','${values.participants[0].prenom}','${values.participants[0].disponnible}')`;
            const monReunion = `'${values.createur_nom}',
             '${values.createur_prenom}',
             '${values.createur_email}',
             '${values.reunion_id}',
              [${strInsParticipant}],
             '${values.date_debut}',
             '${values.date_fin}',
             '${values.description}'`;
            console.log(monReunion);
            yield this.queryDBRaw(`insert into \`${this.dataset}.${table}\` values (${monReunion});`);
        });
    }
    getSchema(table) {
        return __awaiter(this, void 0, void 0, function* () {
